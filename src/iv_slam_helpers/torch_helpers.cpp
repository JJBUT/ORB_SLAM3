...
std::string GetImageType(const cv::Mat& img, bool more_info) {
  ...
  if (more_info) {
    std::cout << "Depth: " << img.depth() << " Channels: " << img.channels()
              << std::endl;
  }
...
at::Tensor TransposeTensor(at::Tensor& tensor, c10::IntArrayRef dims) {
  tensor = tensor.permute(dims);

  return tensor;
}

at::Tensor CVImgToTensor(cv::Mat& img, bool unsqueeze, int unsqueeze_dim) {
  at::Tensor tensor_image =
      torch::from_blob(img.data,
                       {img.rows, img.cols, 3},
                       at::kFloat);  // TODO - at::kByte instead?
...
cv::Mat ToCvImage(at::Tensor& tensor) {
  ...
  try {
    cv::Mat output_mat(
        ...
        CV_8UC1,
        tensor.data_ptr<uchar>());  // TODO - CV_32FC1, tensor.data_ptr<float>()
                                    // instead?
