...
void LocalMapping::CreateNewMapPoints() {
  ...
  for (size_t i = 0; i < vpNeighKFs.size(); i++) {
    ...
    matcher.SearchForTriangulation(
        mpCurrentKeyFrame, pKF2, F12, vMatchedIndices, false, bCoarse);
  ...
  if (bFIBA) {
    if (priorA != 0.f)
      Optimizer::FullInertialBA(
          mpAtlas->GetCurrentMap(), 100, false, 0, NULL, true, priorG, priorA);
    else
      Optimizer::FullInertialBA(
          mpAtlas->GetCurrentMap(), 100, false, 0, NULL, false);
  }
