...
bool LoopClosing::NewDetectCommonRegions() {
  ...
  if (mnLoopNumCoincidences > 0) {
    ...
    g2o::Sim3 gScl(Converter::toMatrix3d(mTcl.rowRange(0, 3).colRange(0, 3)),
                   Converter::toVector3d(mTcl.rowRange(0, 3).col(3)),
                   1.0);
...
bool LoopClosing::DetectAndReffineSim3FromLastKF(
  ...
  if (nNumProjMatches >= nProjMatches) {
    ...
    g2o::Sim3 gSwm(Converter::toMatrix3d(mTwm.rowRange(0, 3).colRange(0, 3)),
                   Converter::toVector3d(mTwm.rowRange(0, 3).col(3)),
                   1.0);
    ...
    if (numOptMatches > nProjOptMatches) {
      g2o::Sim3 gScw_estimation(
          ...
          Converter::toVector3d(mScw.rowRange(0, 3).col(3)),
          1.0);
            ...
            if (numProjOptMatches >= nProjOptMatches) {
              ...
              // for(int j=0; j<vpCurrentCovKFs.size(); ++j)
              {
                ...
                g2o::Sim3 gSjc(
                    ...
                    Converter::toVector3d(mTjc.rowRange(0, 3).col(3)),
                    1.0);
...
void LoopClosing::MergeLocal() {
  ...
  int numTemporalKFs = 15;  // TODO (set by parameter): Temporal KFs in the
  ...
  g2o::Sim3 g2oNonCorrectedSwc(
      Converter::toMatrix3d(Rwc), Converter::toVector3d(twc), 1.0);
  ...
  if (mpTracker->mSensor == System::IMU_MONOCULAR ||
    ...
    Verbose::PrintMess("MERGE-VISUAL: Visual-Inertial",
                       Verbose::VERBOSITY_DEBUG);
    Optimizer::MergeInertialBA(mpLocalMapper->GetCurrKF(),
                               mpMergeMatchedKF,
                               &bStop,
                               mpCurrentKF->GetMap(),
                               vCorrectedSim3);
  ...
  for (KeyFrame* pKFi : spLocalWindowKFs) {
    ...
    for (int j = 0; j < vpMPs.size(); ++j) {
      ...
      if (bIsInImage) {
        ...
        cv::circle(
            img_i, point_img.pt, 1 /*point_img.octave*/, cv::Scalar(0, 255, 0));
        cv::line(img_i, point_img.pt, reproj_p, cv::Scalar(0, 0, 255));
        num_points++;
      } else {
        // cout << "Reproj out of the image" << endl;
        cv::circle(
            img_i, point_img.pt, point_img.octave, cv::Scalar(0, 0, 255));
      }
    }
